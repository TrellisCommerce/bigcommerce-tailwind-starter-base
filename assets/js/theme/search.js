...
export default class Search extends CatalogPage {
  ...
  showProducts(navigate = true) {
    ...
    $('[data-content-results-toggle]').removeClass(
  ...
  showContent(navigate = true) {
    ...
    $('[data-product-results-toggle]').removeClass(
      'navBar-action-color--active',
    );
  ...
  onReady() {
    ...
    const $categoryTreeContainer = $searchForm.find(
      '[data-search-category-tree]',
    );
    ...
    const validator = this.initValidation($searchForm).bindValidation(
      $searchForm.find('#search_query_adv'),
    );
