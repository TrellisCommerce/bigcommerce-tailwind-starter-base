...
export default class GiftCertificate extends PageManager {
  constructor(context) {
    ...
    const $customAmounts = $purchaseForm.find(
      'input[name="certificate_amount"]',
    );
    ...
    if ($customAmounts.length) {
      ...
      const insertFormattedAmountsIntoErrorMessage = (
        message,
        ...amountRange
      ) => {
    ...
    purchaseValidator.add([
      ...
      {
        selector:
          '#gift-certificate-form input[name="certificate_theme"]:first-of-type',
        triggeredBy: '#gift-certificate-form input[name="certificate_theme"]',
        validate: (cb) => {
          const val = $purchaseForm
            .find('input[name="certificate_theme"]:checked')
            .val();
    ...
    $('#gift-certificate-preview').click((event) => {
      ...
      const previewUrl = `${$(event.currentTarget).data(
        'previewUrl',
