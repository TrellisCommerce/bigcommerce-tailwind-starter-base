...
export default class Account extends PageManager {
    ...
    if ($invoiceButton.length) {
      $invoiceButton.on('click', () => {
        ...
        window.open(
          url,
          'orderInvoice',
          `width=900,height=650,left=${left},top=${top},scrollbars=1`,
        );
      });
    }
        ...
        styles: {
          ...
          submitButton: {
            ...
            '&:hover': {
              ...
              backgroundColor:
                themeSettings['button--primary-backgroundColorHover'],
              borderColor:
                themeSettings['button--primary-backgroundColorHover'],
            },
            '&:active': {
              ...
              backgroundColor:
                themeSettings['button--primary-backgroundColorActive'],
              borderColor:
                themeSettings['button--primary-backgroundColorActive'],
            },
            '&[disabled]': {
              backgroundColor:
                themeSettings['button--disabled-backgroundColor'],
  ...
  initReorderForm($reorderForm) {
    $reorderForm.on('submit', (event) => {
      const $productReorderCheckboxes = $(
        '.account-listItem .form-checkbox:checked',
      );
  ...
  initAddressFormValidation($addressForm) {
    ...
    if ($stateElement) {
      ...
      stateCountry($stateElement, this.context, (err, field) => {
        ...
        if ($field.is('select')) {
          $last = field;
          Validators.setStateCountryValidation(
            addressValidator,
            field,
            this.validationDictionary.field_not_blank,
          );
        } else {
  ...
  initPaymentMethodFormValidation($paymentMethodForm) {
    ...
    const $stateElement = $(
      `${paymentMethodSelector} [data-field-type="State"]`,
    );
    ...
    let cardType;
    $(`${paymentMethodSelector} input[name="credit_card_number"]`).on(
      'keyup',
      ({ target }) => {
        cardType = creditCardType(target.value);
        if (cardType) {
          $(`${paymentMethodSelector} img[alt="${cardType}"]`)
            .siblings()
            .css('opacity', '.2');
        } else {
    ...
    CCValidators.setCreditCardNumberValidation(
      paymentMethodValidator,
      `${paymentMethodSelector} input[name="credit_card_number"]`,
      this.context.creditCardNumber,
    );
    CCValidators.setExpirationValidation(
      paymentMethodValidator,
      `${paymentMethodSelector} input[name="expiration"]`,
      this.context.expiration,
    );
    CCValidators.setNameOnCardValidation(
      paymentMethodValidator,
      `${paymentMethodSelector} input[name="name_on_card"]`,
      this.context.nameOnCard,
    );
    CCValidators.setCvvValidation(
      paymentMethodValidator,
      `${paymentMethodSelector} input[name="cvv"]`,
      this.context.cvv,
      () => cardType,
    );

    // Set of credit card format
    CCFormatters.setCreditCardNumberFormat(
      `${paymentMethodSelector} input[name="credit_card_number"]`,
    );
    CCFormatters.setExpirationFormat(
      `${paymentMethodSelector} input[name="expiration"]`,
    );
    ...
    $paymentMethodForm.on('submit', (event) => {
      ...
      if (paymentMethodValidator.areAll('valid')) {
        ...
        const data = _.reduce(
          $paymentMethodForm.serializeArray(),
          (obj, item) => {
            const refObj = obj;
        ...
        const state =
          country &&
          _.find(country.states, ({ value }) => value === data.state);
  ...
  registerEditAccountValidation($editAccountForm) {
    ...
    if ($passwordElement && $password2Element) {
      const { password: enterPassword, password_match: matchPassword } =
