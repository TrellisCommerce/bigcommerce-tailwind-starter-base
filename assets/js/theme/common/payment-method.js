...
export const creditCardType = (value) =>
  creditcards.card.type(creditcards.card.parse(value), true);
  ...
  setCreditCardNumberFormat: (field) => {
    if (field) {
      $(field).on('keyup', ({ target }) => {
        ...
        refTarget.value = creditcards.card.format(
          creditcards.card.parse(target.value),
        );
      });
    }
  },
  ...
  setCreditCardNumberValidation: (validator, field, errorMessage) => {
    if (field) {
      validator.add({
        ...
        validate: (cb, val) => {
          const result =
            val.length && creditcards.card.isValid(creditcards.card.parse(val));
