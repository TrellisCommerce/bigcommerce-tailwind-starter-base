...
export class Collapsible {
  ...
  get isCollapsed() {
    return (
      this.$target.is(':hidden') && !this.$target.hasClass(this.openClassName)
    );
  }
  ...
  _getToggleAriaLabelText($toggle) {
    const $textToggleChildren = $toggle
      .children()
      .filter((__, child) => $(child).text().trim());
    const $ariaLabelTarget = $textToggleChildren.length
      ? $textToggleChildren.first()
      : $toggle;
 ...
 * collapsibleFactory();
 */
export default function collapsibleFactory(
  ...
  return $collapsibles
    .map((index, element) => {
      ...
      const collapsible = new Collapsible(
        $toggle,
        $(targetId, overrideOptions.$context),
        options,
      );
