  ...
  describe('Formatters', () => {
    ...
    describe('setCreditCardNumberFormat', () => {
      it('should be formatting the credit card number in a corresponding credit card type format', () => {
        Formatters.setCreditCardNumberFormat(
          'form input[name="credit_card_number"]',
        );
        const input = $('form input[name="credit_card_number"]');

        expect(input.val('370000000000000').trigger('keyup').val()).toEqual(
          '3700 000000 00000',
        );
        expect(input.val('38000000000000').trigger('keyup').val()).toEqual(
          '3800 000000 0000',
        );
        expect(input.val('6011000000000000').trigger('keyup').val()).toEqual(
          '6011 0000 0000 0000',
        );
        expect(input.val('5163000000000000').trigger('keyup').val()).toEqual(
          '5163 0000 0000 0000',
        );
        expect(input.val('4111000000000000').trigger('keyup').val()).toEqual(
          '4111 0000 0000 0000',
        );
      });
    });
  ...
  describe('Validators', () => {
    describe('setCreditCardNumberValidation', () => {
      it('should have invalid input credit card number', () => {
        ...
        const validator = {
          add: ({ validate }) => validate(callback, '4444 3333 2222'),
        };
      ...
      it('should have valid input credit card number', () => {
        const callback = jasmine.createSpy();
        const validator = {
          add: ({ validate }) => validate(callback, '4444 3333 2222 1111'),
        };
        Validators.setCreditCardNumberValidation(validator, 'selector');
    ...
    describe('setExpirationValidation', () => {
      it('should have invalid input expiration date', () => {
        const callback = jasmine.createSpy();
        const validator = {
          add: ({ validate }) => validate(callback, '01/17'),
        };
      ...
      it('should have valid input expiration date', () => {
        const callback = jasmine.createSpy();
        const validator = {
          add: ({ validate }) => validate(callback, '12/25'),
        };
        Validators.setExpirationValidation(validator, 'selector');
    ...
    describe('setNameOnCardValidation', () => {
      ...
      it('should have valid input name on card', () => {
        const callback = jasmine.createSpy();
        const validator = {
          add: ({ validate }) => validate(callback, 'Foo Bar'),
        };
        Validators.setNameOnCardValidation(validator, 'selector');
