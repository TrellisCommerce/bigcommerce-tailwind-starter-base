...
describe('FacetedSearch', () => {
  ...
  describe('refreshView', () => {
    ...
    it('should re-init price range validator', function () {
      ...
      expect(Validators.setMinMaxPriceValidation).toHaveBeenCalledWith(
        facetedSearch.priceRangeValidator,
        jasmine.any(Object),
        options.validationErrorMessages,
      );
    });
  });
  ...
  describe('updateView', () => {
    ...
    it('should refresh view', function () {
      jest
        .spyOn(api, 'getPage')
        .mockImplementation(function (url, options, callback) {
  ...
  describe('when price range form is submitted', () => {
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(facetedSearch.priceRangeValidator, 'areAll')
    ...
    it('should set `min_price` and `max_price` query param to corresponding form values if form is valid', () => {
      ...
      expect(urlUtils.goToUrl).toHaveBeenCalledWith(
  ...
  describe('when price range form is submitted with other facets selected', () => {
    ...
    it('send `min_price` and `max_price` query params if form is valid', () => {
      ...
      expect(urlUtils.goToUrl).toHaveBeenCalledWith(
        '/?brand[]=item1&brand[]=item2&min_price=0&max_price=50',
      );
    });
  });
